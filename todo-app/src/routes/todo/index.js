...
export default class Todo extends Component {
	...
	createTodo = async () => {
		...
		try {
			const result = await axios.post('http://localhost:9000/api/todo', temp);
	...
	listTodo = async () => {
		try {
			const result = await axios.get('http://localhost:9000/api/todo');
	...
	render({},{ todos, isCreate, createNew }) {
		return (
			<div class={style.home}>
				...
				<h1 style="margin-left:70px;">MySql Todo App </h1>
...
class TodoList extends Component{
	...
	updateTodo = async (e) => {
		...
		this.setState({ isEditing: !this.state.isEditing });
		try {
			const result = await axios.put(`http://localhost:9000/api/todo/${this.props.value.id}`,
	...
	destroyTodo = async () => {
		...
		try {
			await axios.delete(`http://localhost:9000/api/todo/${this.props.value.id}`);
			this.props.onDelete();
		}
		catch (error){
			console.log(error);
	...
	handleStatus = async () => {
		...
		try {
			const result = await axios.put(`http://localhost:9000/api/todo/${this.state.id}`, { status: this.state.status });
			console.log(result.data);
		}
		catch (error){
			console.log(error);
